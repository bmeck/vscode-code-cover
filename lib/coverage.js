	...
	perform(function *() {
		...
		for (const file of config.coverage) {
			...
			if (ext === '.info') {
				...
				for (const line of matchLCOV(activeFile, dir, d, mappings)) {
					if (line.type === 'line') {
				...
				for (const line of matchJSON(activeFile, dir, json, mappings)) {
					if (line.type === 'line') {
						lines.push(line.line);
					}
					if (line.type === 'branch') {
...
function* matchJSON(activeFile, rootPath, coverage, mappings) {
	const found = new Set();
	for (const file of Object.keys(coverage)) {
		...
		for (const id in fileinfo.b) {
			...
			if (fileinfo.b[id].indexOf(0) === -1) continue;
			if (activeFile === resolvedFile) {
...
function* matchLCOV(activeFile, rootPath, coverage, mappings) {
	const found = new Set();
